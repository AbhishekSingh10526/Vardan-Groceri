...
def add_product():
    ...
    if c and c != '' and c.isdigit():
        if models.Category.query.get(int(c)):
            category_id = int(c)
...
def add_product_post():
    ...
    product = models.Product(name=name, quantity=quantity, price=price, category=category, man_date=man_date)
...
def delete_product(id):
    ...
    if not product:
        ...
        return redirect(url_for('admin'))
    return render_template('product/delete.html', user=models.User.query.get(session['user_id']), product=product)
...
def edit_category(id):
    return render_template('category/edit.html', user=models.User.query.get(session['user_id']), category=models.Category.query.get(id))
...
def delete_category(id):
    ...
    if not category:
        flash('Category does not exist.')
        return redirect(url_for('admin'))
    return render_template('category/delete.html', user=models.User.query.get(session['user_id']), category=category)
...
def index():
    ...
    if not parameter or not query:
        return render_template('index.html', user=user, categories=models.Category.query.all(), parameters=parameters)
    if parameter == 'category':
        categories = models.Category.query.filter(models.Category.name.like('%' + query + '%')).all()
    ...
    if parameter == 'product':
        return render_template('index.html', user=user, categories=models.Category.query.all(), name=query, query=query, parameter=parameter, parameters=parameters)
    if parameter == 'price':
        return render_template('index.html', user=user, categories=models.Category.query.all(), price=float(query), query=query, parameter=parameter, parameters=parameters)
...
def add_to_cart(product_id):
    ...
    cart = models.Cart.query.filter_by(user_id=session['user_id']).filter_by(product_id=product_id).first()
    if cart:
        ...
        return redirect(url_for('index'))
    cart = models.Cart(user_id=session['user_id'], product_id=product_id, quantity=quantity)
    db.session.add(cart)
    db.session.commit()
    flash('Product added to cart successfully.')
    return redirect(url_for('index'))
...
@auth_required
def place_order():
    items = models.Cart.query.filter_by(user_id=session['user_id']).all()
    if not items:
        flash('Cart is empty.')
        return redirect(url_for('cart'))
    for item in items:
    ...
    for item in items:
        ...
        order = models.Order(product_id=item.product_id, quantity=item.quantity, price=item.product.price, transaction=transaction)
